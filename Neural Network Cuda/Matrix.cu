...
void Matrix::copyHostToDevice()
{
	...
	{
		throw NNException("Cannot copy host data to unallocated memory on device.");
	}
}
...
void Matrix::copyDeviceToHost()
{
	if (device_allocated && host_allocated)
	{
		...
		NNException::throwIfDeviceErrorsOccurred("Cannot copy CUDA device data to host.");
	}
	else
	{
		throw NNException("Cannot copy device data to unallocated memory on host.");
	}
}
